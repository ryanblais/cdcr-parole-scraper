...
class SR_SCRAPER:
    ...
    def get_monthly_urls_for_hearing_schedules(self):
        ...
        return page_contents.get_schedule_links_following_header_string(
            id=self.get_property_value(URLType.HEARING_SCHEDULE, h3_id_property_key),
            innerString=self.get_property_value(URLType.HEARING_SCHEDULE, h3_string_property_key))
    ...
    def get_monthly_urls_for_hearing_results(self):
        ...
        page_contents = self.process_url(base_url)
        return page_contents.get_schedule_links_following_header_string(
            innerString= self.get_property_value(URLType.HEARING_RESULTS, h3_string_property_key))
